...
function CharacterCreationPage({ store, setStore, user, campaign = null, mode = "personal", onCancel, onComplete }) {
  ...
  const toggleCharacteristic = (item) => {
    if (selectedCharacteristics.includes(item.id)) { updateNested("characteristics", { selected: selectedCharacteristics.filter((id) => id !== item.id) }); return; }
    if (!evaluateCharacteristicRequirement(item.requirements, aptitudes, tug.assimilationLevel) || characteristicsCost + item.cost + aptitudeXpCost > 7) return;
    updateNested("characteristics", { selected: [...selectedCharacteristics, item.id] });
  };
...
function CampaignReferenceNav({ campaignId, active, onBack }) {
  ...
  return <nav className="campaign-reference-nav" aria-label="Navegação da campanha">
    {links.map(([key, label, path]) => <button type="button" key={key} className={active === key ? "is-active" : ""} onClick={() => key === "participants" ? onBack() : go(path)}>{label}</button>)}
  </nav>;
}

function ReferencePageHeader({ campaign, title, description, onBack, active, count }) {
  return <>
    <header className="campaign-page-header campaign-detail-header">
      <div><button type="button" className="campaign-back-link" onClick={onBack}>← {campaign.name}</button><span className="eyebrow">COMPÊNDIO DA CAMPANHA</span><h1>{title}</h1><p>{description}</p></div>
      {count !== undefined && <span className="campaign-role campaign-role--player">{count} entradas</span>}
    </header>
    <CampaignReferenceNav campaignId={campaign.id} active={active} onBack={onBack} />
  </>;
}
...
function CampaignCharacteristicsPage({ store, user, campaignId, onBack }) {
  ...
  return <div className="campaign-page reference-page"><ReferencePageHeader campaign={campaign} title="Características" description="Consulta rápida das opções oficiais para criação e evolução." active="characteristics" count={visibleCharacteristics.length} /><main className="campaign-page-content">
...
function CampaignAssimilationsPage({ store, user, campaignId, onBack }) {
  ...
  return <div className="campaign-page reference-page"><ReferencePageHeader campaign={campaign} title="Assimilações" description="Catálogo oficial organizado por família, nível e habilidades internas." active="assimilations" count={visibleAssimilations.length} /><main className="campaign-page-content">
...
function CampaignPage({ store, setStore, user, campaignId, onBack, onOpenItems, onOpenCharacteristics, onOpenAssimilations, onOpenCharacter, onCreateCharacter, onUsePersonal }) {
  ...
  return <div className="campaign-page campaign-detail-page">
    <header className="campaign-page-header campaign-detail-header"><div><button type="button" className="campaign-back-link" onClick={onBack}>← Minhas campanhas</button><span className="eyebrow">CAMPANHA</span><h1>{campaign.name}</h1><p>{memberships.length} participantes · {role === "master" ? "Você é o mestre" : "Você participa como jogador"}</p></div><div className="campaign-detail-header-actions"><div className="campaign-reference-actions"><button type="button" className="campaign-primary-btn" onClick={() => onOpenItems(campaignId)}>Itens</button><button type="button" className="campaign-secondary-btn" onClick={() => onOpenCharacteristics(campaignId)}>Características</button><button type="button" className="campaign-secondary-btn" onClick={() => onOpenAssimilations(campaignId)}>Assimilações</button></div><span className={`campaign-role campaign-role--${role}`}>{role === "master" ? "MESTRE" : "JOGADOR"}</span>{role === "master" && <div className="campaign-code-box"><span>Código da campanha</span><strong>{campaign.joinCode}</strong><button type="button" onClick={copyJoinCode}>{copyLabel}</button></div>}</div></header>
    <main className="campaign-page-content campaign-participants-content">
